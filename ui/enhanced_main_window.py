"""
Enhanced Main Window with Intelligent Training Integration
Updated to use EnhancedModelPanel and integrate intelligent training features
"""
...
from ui.enhanced_model_panel import EnhancedModelPanel
...
class EnhancedMainWindow(QMainWindow):
    def __init__(self):
        ...
        # Initialize intelligent training system
        self.intelligent_training = IntelligentTrainingIntegration(self)
    ...
    def _build_ui(self):
        ...
        self.model_panel = EnhancedModelPanel()  # Using enhanced model panel
    ...
    def _make_welcome_page(self) -> QWidget:
        ...
        welcome_card.setMaximumWidth(750)
        welcome_card.setMinimumWidth(300)
        ...
        # Add intelligent training features showcase
        features_lbl = QLabel("🤖 Intelligent Features: AutoML • Quality Checking • Model Selection • Optimization")
        features_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        features_lbl.setStyleSheet("color: #818CF8; font-size: 10pt; font-weight: 600;")
        ...
        # Wrap in ScrollArea for responsiveness
        scroll = QScrollArea()
    ...
    def _make_wizard_page(self, panel, title, back_text, next_text, back_idx, next_idx) -> QWidget:
        ...
        lbl_title.setStyleSheet(
            "font-size: 14pt; font-weight: 800; color: #818CF8; "
    ...
    def _make_training_page(self) -> QWidget:
        # Create a container widget that will be scrolled
        container = QWidget()
    ...
    def _build_metric_strip(self) -> QFrame:
        ...
        container.setFixedHeight(135) # Increased from 120
        ...
        for name, init, color in metrics:
            ...
            cly.setContentsMargins(12, 10, 12, 10) # Reduced vertical margin
            cly.setSpacing(4) # Tighter spacing between label and value
    ...
    def _connect_signals(self):
        # Enhanced training panel connections with intelligent training
        ...
        # Connect intelligent training signals
        self.intelligent_training.sig_preparation_completed.connect(self._on_preparation_completed)
        self.intelligent_training.sig_training_started.connect(self._on_intelligent_training_started)
        self.intelligent_training.sig_training_progress.connect(self._on_intelligent_training_progress)
        self.intelligent_training.sig_training_completed.connect(self._on_intelligent_training_completed)
        self.intelligent_training.sig_intelligent_feature_activated.connect(self._on_intelligent_feature_activated)
        self.intelligent_training.sig_model_selected.connect(self._on_model_selected)
        self.intelligent_training.sig_optimization_applied.connect(self._on_optimization_applied)
        self.intelligent_training.sig_quality_issue_detected.connect(self._on_quality_issue_detected)
    ...
    def _detect_and_set_hardware(self):
        ...
        # Set hardware info in model panel
        vram_gb = 0
        if torch.cuda.is_available():
            try:
                ...
                pass
        self.model_panel.set_hardware_info(vram_gb)
    ...
    # ── Intelligent Training Integration Methods ──────────────────────────────
    
    def _on_preparation_completed(self, prep_data: dict):
        """Show mandatory confirmation dialog before training proceeds"""
        data_analysis = prep_data.get('data_analysis')
        model_recommendation = prep_data.get('model_recommendation')
        optimizations = prep_data.get('optimizations', [])
        
        # Build text for the dialog
        task_info = f"<b>Task:</b> {self.model_panel.current_task_domain} - {self.model_panel.current_task_type}"
        
        data_info = "<b>Dataset Analysis:</b><br>"
        if data_analysis:
            data_info += f"- Type: {data_analysis.data_type}<br>"
            data_info += f"- Classes: {data_analysis.num_classes}<br>"
            data_info += f"- Samples: {data_analysis.num_samples}<br>"
            data_info += f"- Quality Score: {data_analysis.data_quality_score:.2f}<br>"
            
        model_info = "<b>Selected Model:</b><br>"
        if model_recommendation:
            model_info += f"- Architecture: {model_recommendation.architecture}<br>"
            model_info += f"- Reason: {model_recommendation.reasoning}<br>"
            
        opt_info = "<b>Automatic Adjustments Applied:</b><br>"
        if optimizations:
            for opt in optimizations:
                opt_info += f"- {opt.get('parameter', 'opt')}: {opt.get('old_value')} → {opt.get('new_value')} <i>({opt.get('reason')})</i><br>"
        else:
            opt_info += "- None<br>"
            
        full_text = f"{task_info}<br><br>{data_info}<br>{model_info}<br>{opt_info}<br><b>Do you want to proceed with training?</b>"
        
        # Show Dialog
        reply = QMessageBox.question(
            self,
            "Pre-Training Confirmation (SHADA Pipeline)",
            full_text,
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # User confirmed, start execution thread
            from PyQt6.QtCore import QThread
            import threading
            
            def run_execution():
                try:
                    self.intelligent_training.execute_intelligent_training()
                except Exception as e:
                    self.intelligent_training.sig_training_completed.emit({'error': str(e)})
            
            exec_thread = threading.Thread(target=run_execution, daemon=True)
            exec_thread.start()
        else:
            # User cancelled
            self.log_panel.append_log("❌ Training cancelled by user at confirmation step.", "warn")

    def _on_intelligent_training_started(self):
        """Handle intelligent training start"""
        self.log_panel.append_log("🚀 Intelligent training pipeline started", "info")
        self._set_page(4)  # Go to training page

    def _on_intelligent_training_progress(self, progress: int, message: str):
        """Handle intelligent training progress updates"""
        self.log_panel.append_log(f"📈 Intelligent Training: {progress}% - {message}", "info")

    def _on_intelligent_training_completed(self, results: dict):
        """Handle intelligent training completion"""
        if 'error' in results:
            QMessageBox.critical(self, "Intelligent Training Failed", results['error'])
        else:
            summary = f"Training completed! Final accuracy: {results.get('training_results', {}).get('best_val_accuracy', 0):.3f}"
            QMessageBox.information(self, "Intelligent Training Complete", summary)
            
            # Log intelligent features used
            features = results.get('intelligent_features_used', [])
            self.log_panel.append_log(f"🤖 Intelligent features used: {', '.join(features)}", "success")
            
            # Log optimizations applied
            optimizations = results.get('optimization_summary', {})
            self.log_panel.append_log(f"⚡ Optimizations applied: {optimizations.get('total_optimizations_applied', 0)}", "success")

    def _on_intelligent_feature_activated(self, feature_name: str, description: str):
        """Handle intelligent feature activation"""
        self.log_panel.append_log(f"🤖 {feature_name}: {description}", "info")

    def _on_model_selected(self, model_name: str, reasoning: str):
        """Handle intelligent model selection"""
        self.log_panel.append_log(f"🎯 Intelligent model selection: {model_name}", "info")
        self.log_panel.append_log(f"   Reasoning: {reasoning}", "info")

    def _on_optimization_applied(self, optimization_type: str, details: str):
        """Handle optimization application"""
        self.log_panel.append_log(f"⚡ {optimization_type}: {details}", "info")

    def _on_quality_issue_detected(self, issue_type: str, severity: str):
        """Handle quality issue detection"""
        self.log_panel.append_log(f"🔍 Quality issue detected: {issue_type} ({severity})", "warn")

    def start_intelligent_training(self):
        """Start intelligent training with current configuration"""
        try:
            # Get current configuration
            config = self.model_panel.get_config()
            data_path = self.data_panel.train_path if hasattr(self.data_panel, 'train_path') else ""
            
            if not data_path:
                QMessageBox.warning(self, "No Data Path", "Please select a dataset first.")
                return
            
            # Start intelligent training
            self.log_panel.append_log("🚀 Starting intelligent training pipeline...", "info")
            ...
            def run_intelligent_training():
                try:
                    results = self.intelligent_training.start_intelligent_training(data_path, config)
                    # Results will be emitted via signals
                except Exception as e:
                    self.intelligent_training.sig_training_completed.emit({'error': str(e)})
            
            # Start in background thread
            training_thread = threading.Thread(target=run_intelligent_training, daemon=True)
            training_thread.start()
            
        except Exception as e:
            QMessageBox.critical(self, "Intelligent Training Error", f"Failed to start intelligent training: {str(e)}")


# Integration function to replace original main window
def create_enhanced_main_window() -> EnhancedMainWindow:
    """Create and return the enhanced main window with intelligent training"""
